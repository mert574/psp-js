...
import { IO, USB, USB_ACC, USB_GPS, USB_MIC } from "./nids.js";
...
export function registerIoHLE(kernel: HLEKernel): void {
  ...
  kernel.register(IO.sceIoOpen, (regs, bus) => {
    ...
    const fd = nextFd++;
    openFiles.set(fd, { path, data: fileBytes, position: 0, asyncResult: 0, hasAsyncResult: false });
  ...
  kernel.register(IO.sceIoDevctl, (regs, bus) => {
    ...
    const outPtr = bus.readU32(regs.getGpr(29) + 16);
